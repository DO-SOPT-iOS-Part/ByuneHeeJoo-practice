...
class ViewController: UIViewController {
    ...
    @objc private func registerButtonTap() {
        Task {
            do {
                if let result = try await CheckUserExistService.shared.GetRegisterData(userName: String(self.usreName)) {
                    if result.isExist == true {
                        let alert = UIAlertController(title: "이미 있는 게정", message: "바보", preferredStyle: UIAlertController.Style.alert)
                        let okAction =  UIAlertAction(title: "확인", style: UIAlertAction.Style.default)
                        alert.addAction(okAction)
                        self.present(alert, animated: true)
                    } else {
                        let status = try await RegisterService.shared.PostRegisterData(usreName: self.usreName, password: self.password, nickName: self.nickName)
                        if status == 201 {
